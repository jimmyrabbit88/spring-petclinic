...
//                bat 'docker build -t jimmyrabbit88/pet_clinic_1:2.0.0 .'
                bat 'docker build -t paulcaff/petclinic:2.0.0 .'
            }
        }
        ...
        stage("Deploy Container"){
            steps{
                sshagent(['aws-rem']){
                    ...
                    sh "ssh -o StrictHostKeyChecking=no ec2-user@34.244.114.135 docker run -p 8080:8080 -d --name pet_clinic paulcaff/petclinic:2.0.0"
                }
            }
        }
